...
export const config = {
  ...
  /**
   * Кто и где мы. В норме определяется по примонтированному хосту, эти
   * переменные — способ сказать панели явно, когда автоопределение не подходит
   * (хост не примонтирован, имя в /etc/hostname не то, которое видят соседи).
   * Пусто — определять самой.
   */
  identity: {
    hostname: str('DOCK_HOSTNAME', ''),
    tz: str('DOCK_TZ', ''),
    operator: str('DOCK_OPERATOR', ''),
  },
